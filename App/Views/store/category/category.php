	...
	<div class="pagination-container">
		<?php if ($pagination->totalItems() > $pagination->itemsPerPage()) : ?>
			<div class="pagination">
				<a href="<?php echo url('category?id=') . $categoryId . '&order-by=' . $orderBy . '&sort-by=' . $sortBy . '&page=' . $pagination->prev() ?>">&laquo;</a>
				<?php
				for ($i = 1; $i <= $pagination->last(); $i++) {
					$html = '<a ' . ($i == $pagination->page() ? 'class="active"' : '') . ' href="' . url('category?id=') . $categoryId . '&order-by=' . $orderBy . '&sort-by=' . $sortBy . '&page=' . $i . '">' . $i . '</a>';
					echo $html;
				}
				?>
				<a href="<?php echo url('category?id=') . $categoryId . '&order-by=' . $orderBy . '&sort-by=' . $sortBy . '&page=' . $pagination->next() ?>">&raquo;</a>
			</div>
		<?php endif; ?>
	</div>
</div>